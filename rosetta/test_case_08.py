...
def myTY1(x):
    return 0.5 + (math.sin((hou.hscriptExpression("($F-1)/($NFRAMES-1)")*math.pi*2)+(x*math.pi*2))/2)
...
def funplot(TY):
    ...
    # i*1.0
    copy.parm('tx').setExpression("(lvar(\"CY\")/lvar(\"NCY\")) * 2.0", hou.exprLanguage.Python)
    # TY(i)
